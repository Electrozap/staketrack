...
export default function StakeTrackApp() {
  ...
  const handleResolveBet = async (betId: string, winnerName: string) => {
    const { data, error } = await supabase
      .from("bets")
      .update({ status: "won", winner: winnerName })
      .eq("id", betId)
      .select();
  ...
  const handleSettleUp = async () => {
    ...
    const { data, error } = await supabase
      .from("settlements")
      .insert([{ payer: myName, payee: settleData.payee, amount }])
      .select();
    if (error) alert("Error: " + error.message);
    else if (data) {
  ...
  if (!user) {
    return <LandingPage onSignIn={handleGoogleLogin} />;
  }
    ...
    <div className="app">
      <aside className="sidebar">
        ...
        <nav className="sidebar-nav">
          ...
          <button
            className={`nav-item ${activeView === "settlements" ? "active" : ""}`}
        ...
        <div className="sidebar-footer">
          <div className="user-card flex flex-col items-start w-full">
            ...
            <button
              ...
              className="text-xs text-[var(--color-text-muted)] hover:text-white w-full text-left p-1"
            >
              Sign Out
            </button>
          </div>
        ...
        <main className="main-content">
            ...
            <>
              <div className="kpi-grid mb-8">
                <div className="kpi-card">
                  <div className="kpi-label">Your Bets</div>
                  <div className="kpi-value">{bets.length}</div>
                </div>
                <div className="kpi-card">
                  <div className="kpi-label">Pending Action</div>
                  <div className="kpi-value text-[var(--color-gold)]">{pendingCount}</div>
                </div>
                <div className="kpi-card">
                  <div className="kpi-label">Bets Won</div>
                  <div className="kpi-value text-[var(--color-success)]">{wonCount}</div>
                </div>
                <div className="kpi-card">
                  <div className="kpi-label">Total Pot Volume</div>
                  <div className="kpi-value">&#8377;{totalPot}</div>
                </div>
              </div>
                  ...
                  <tbody>
                    {bets.slice(0, 4).map((b) => (
                      <tr key={b.id}>
                        ...
                        <td>
                          <span className={`badge badge-${b.sport.toLowerCase()}`}>{b.sport}</span>
                        </td>
                        <td className="amount">&#8377;{b.stake}</td>
                        <td>
                          <span className={`badge badge-${b.status}`}>
                            {b.status === "won" ? `Won by ${b.winner}` : b.status}
                          </span>
                        </td>
                      </tr>
                    ))}
              ...
              <div className="table-wrap">
                <table className="data-table">
                  ...
                  <tbody>
                    {bets.length === 0 ? (
                      <tr>
                        <td colSpan={6} className="text-center p-8 text-[var(--color-text-muted)]">
                          No bets found. Create one.
                        </td>
                      </tr>
                    ) : (
                      bets.map((b) => (
                        <tr key={b.id}>
                          ...
                          <td>
                            <span className={`badge badge-${b.sport.toLowerCase()}`}>
                              {b.sport}
                            </span>
                          </td>
                          <td>
                            ...
                            {b.status === "pending" && (
                              <div className="flex gap-2">
                                {(b.participants || []).map((p: string) => (
                                  <button
                                    key={p}
                                    onClick={() => handleResolveBet(b.id, p)}
                                    className="btn btn-ghost text-xs py-1 px-2 border border-[var(--color-border)]"
                                  >
                                    {p} won
                                  </button>
                                ))}
          ...
          {activeView === "settlements" && (
            <div className="flex flex-col gap-8">
              ...
              <div>
                <div className="section-header">
                  <div className="section-title text-[var(--color-success)]">You are owed</div>
                </div>
                <div className="lb-list bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl overflow-hidden shadow-sm">
                  {youAreOwed.length === 0 ? (
                    ...
                    youAreOwed.map((b) => (
                      <div
                        key={b.name}
                        className="lb-item flex items-center justify-between p-4 border-b border-[var(--color-divider)]"
                      >
                        <div className="flex items-center gap-4">
                          <div className="avatar w-9 h-9 text-xs">{b.name.charAt(0)}</div>
                          <div className="font-medium">{b.name}</div>
                        </div>
                        <div className="font-bold text-lg text-[var(--color-success)]">
                          &#8377;{b.balance}
                        </div>
                      </div>
                    ))
              ...
              <div>
                <div className="section-header">
                  <div className="section-title text-[var(--color-error)]">You owe</div>
                </div>
                <div className="lb-list bg-[var(--color-surface)] border border-[var(--color-border)] rounded-xl overflow-hidden shadow-sm">
                  {youOwe.length === 0 ? (
                    <div className="p-6 text-center text-[var(--color-text-muted)]">
                    ...
                    youOwe.map((b) => (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-4">
                          <div className="font-bold text-lg text-[var(--color-error)]">
                            &#8377;{Math.abs(b.balance)}
                          </div>
                          <button
                            onClick={() => {
                              setSettleData({
                                payee: b.name,
                                amount: Math.abs(b.balance).toString(),
                              });
        ...
        >
          <div className="modal">
            ...
            <div className="modal-body">
              <div className="form-group">
                ...
                <input
                  ...
                  placeholder="e.g. MI vs CSK – Runs in first 6 overs"
                />
              </div>
              ...
              <div className="form-group">
                ...
                <div style={{ display: "flex", gap: "8px" }}>
                  <input
                    ...
                    placeholder="Friend's exact Google Name"
                  />
                  <button className="btn btn-ghost" onClick={handleAddParticipant}>
                    <UserPlus size={14} />
                  </button>
                </div>
                <div style={{ display: "flex", gap: "8px", marginTop: "8px", flexWrap: "wrap" }}>
                  {newParticipants.map((p) => (
                    <span key={p} className="badge badge-custom flex items-center">
                      {p}{" "}
                      {p !== (user.user_metadata?.full_name || "Me") && (
                        <X
                          size={12}
                          className="ml-1 cursor-pointer"
                          onClick={() => setNewParticipants(newParticipants.filter((x) => x !== p))}
                        />
                      )}
                    </span>
                  ))}
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-primary" onClick={handleCreateBet}>
                Create Bet
              </button>
            </div>
          </div>
      ...
      {isSettleModalOpen && (
        <div
          className="modal-overlay open"
          onClick={(e) => e.target === e.currentTarget && setIsSettleModalOpen(false)}
        >
          <div className="modal" style={{ maxWidth: "400px" }}>
            <div className="modal-header">
              <h2 className="modal-title flex items-center gap-2">
                <IndianRupee size={20} /> Settle Debt
              </h2>
              <button className="btn-icon" onClick={() => setIsSettleModalOpen(false)}>
                <X size={18} />
              </button>
            </div>
            <div className="modal-body">
              <p className="text-sm text-[var(--color-text-muted)] mb-2">
                Record a payment sent to <strong>{settleData.payee}</strong>.
              </p>
              <div className="form-group">
            ...
            <div className="modal-footer">
              <button className="btn btn-ghost" onClick={() => setIsSettleModalOpen(false)}>
                Cancel
              </button>
              <button className="btn btn-primary" onClick={handleSettleUp}>
                Confirm Payment
              </button>
            </div>
          </div>
